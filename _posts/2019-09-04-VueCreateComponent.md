...
* 创建组件中 data必须是一个function  因为需要形成不被其他组件影响的 作用域

* 每个组件的模板 必须只有一个根元素 就是所 组件外部必须包含一个元素
  ...
  <body>
      <div id = "vm">
          ...
          <!--组件复用-->
          <!--由于 该组件的data选项是一个独立的作用域 所以即使调用同一个组件 也不会互相影响-->
          <button-counter></button-counter>
  ...
  <script>
      Vue.component('button-counter',{
          ...
          template : `
          <button @click = 'addFunc(num)'> 点击{{num}} </button>
          `
      })
      var vm = new Vue({
          el : "#vm"
...
* 每个组件模板必须只有一个根元素 就是所 组件外部必须包含一个元素
...
<script>
    ...
    var component1 = {
        ...
        `,// 需要包含根元素,且 里面只允许添加component1作用域内部的局部组件和全局组件
        // 不能使用 因为 这两个组件是局部组件 相互不可调用
        components : {
            "haha" : {
                template : `<span>我是组件component1中的同步组件1</span>`
            }
        }
    }
...
<body>
      <div id= "vm">
        <par-component title = "我是父组件" ></par-component>
        <par-component :title = "title" ></par-component>
      </div> 

</body>
<script>
    Vue.component("par-component",{
        data : function(){                 //这里面是 子组件属性content的值, 但是里面的组件
        // 无论多少,数据永远在一个data 中
                    return {
                        content : "我是动态的子组件"
                    }
        ...
        template : `
            <div>
                <h2>{{title}}</h2> 
                <son-component content = "我是子组件"></son-component>   
                <son-component :content = "content"></son-component>   
            </div>
        `,
        components : {            
            "son-component" : {
